...
class ImageDataset(Dataset):
    def __init__(self, image_root, trf_root, patch_size=5, batch_size = 1, patches_per_step=1, mode = None):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "Train":
            selected_indices = np.random.choice(total_indices, size=patches_per_step, replace=False) #False: 비복원 추출
        ...
        if self.mode == "Test":
            return patches.astype(np.float64), self.trf.astype(np.float64), self.images_list[idx]
        else:
...
class PixelConditionEncoder(nn.Module):
    def __init__(self, in_channels=1, cardinality=2, kernel_size=3):
        super(PixelConditionEncoder, self).__init__()

        self.split_convs = nn.ModuleList([
            nn.Conv2d(in_channels, 1, kernel_size=kernel_size, padding=kernel_size // 2)
            for _ in range(cardinality)
        ])

        self.merge_fc = nn.Linear(cardinality, 1)
        self.pool = nn.AdaptiveAvgPool2d((1, 1))
        self.act = nn.SiLU()

    def forward(self, x):
        # 각 branch는 (B, 1, H, W) → (B, 1, H, W)
        features = [self.pool(self.act(conv(x))) for conv in self.split_convs]  # 각: (B, 1, 1, 1)
        features = torch.cat(features, dim=1)  # (B, cardinality, 1, 1)
        features = features.view(features.size(0), -1)  # (B, cardinality)
        out = self.act(self.merge_fc(features))  # (B, 1)
        return out
...
def Train(num_epochs, train_loader, val_loader, device, patch_size=5, kernel_size=5, patches_per_step=None):
    mode = "Train"
    ...
    for epoch in range(num_epochs):
        ...
        for img_patches, trf in tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
            ...
            loss = L2loss(torch.log2(predicted_pixel+1)/15, center_pixel)
            
            if i < 1:
                # print(center_pixel)
                # print(np.min(DEMs.cpu().detach().numpy()))
                print(predicted_pixel[10])
                print(2**(center_pixel[10]*15)-1)
                print("===================================")
                i+=1
            ...
            train_loss += loss.item()* img_patches.size(0)
            total_count += img_patches.size(0)

        average_loss = train_loss / total_count
        ...
        if epoch % 3 == 0:
            try:
                ...
                }, current_path + '/{}/checkpoint/DEM4HRI/{}.pth'.format('0_#model', epoch))
            except:
                os.makedirs(current_path + '/{}/checkpoint/DEM4HRI/'.format('0_#model'))
                torch.save({
                    'encoder': encoder.state_dict(),
                    'cond_encoder': cond_encoder.state_dict(),
                    'decoder': decoder.state_dict()
        ...
        with torch.no_grad():
            for img_patches, trf in tqdm(val_loader, desc=f"Epoch {epoch+1}/{num_epochs}"):
                ...
                val_loss += loss.item()* img_patches.size(0)
                total_count += img_patches.size(0)
                
            Val_avg_loss = val_loss / total_count
...
if __name__ == '__main__':
    ...
    slice_num = 39546
    patches_per_step = int(patch_num(image_size=image_size, patch_size=patch_size) // slice_num)
    num_epochs= 100
