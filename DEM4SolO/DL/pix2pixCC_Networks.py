...
class PatchDiscriminator(nn.Module):
    def __init__(self, opt):
        ...
        #----------------------------------------------------------------------
        if opt.ch_balance > 0:
            ch_ratio = np.float32(opt.input_ch)/np.float32(opt.target_ch)
            ch_ratio *= opt.ch_balance
            if ch_ratio > 1:
                input_channel = opt.input_ch + opt.target_ch*np.int32(ch_ratio)                            
            elif ch_ratio < 1:
                input_channel = opt.input_ch*np.int32(1/ch_ratio) + opt.target_ch
            else:
...
class Loss(object):
    ...
    def __call__(self, D, G, input, target):
        ...
        if self.opt.ch_balance > 0:  
            ...
            if ch_ratio > 1:
                for _ in range(np.int32(ch_ratio)-1):
