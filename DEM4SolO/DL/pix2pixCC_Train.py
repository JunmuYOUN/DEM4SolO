...
setproctitle('YOUN-211A gfdf64-2 10 5')
#==============================================================================
            ...
            if opt.val_during_train and ((current_step//batch_sz) % (save_freq//batch_sz) == 0):
                ...
                for input, target, _, name in tqdm(test_data_loader):
                    ...
                    if opt.display_scale != 1:
                        sav_fake = np.clip(np_fake*np.float32(opt.display_scale), -1, 1)
                        sav_real = np.clip(np_real*np.float32(opt.display_scale), -1, 1)
                    else:
                        sav_fake = np_fake
                        sav_real = np_real
